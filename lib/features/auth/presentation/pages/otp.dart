...
import 'package:ramro_postal_service/features/auth/data/models/resend_otp_request.dart';
            ...
            child: Column(
              ...
              children: [
                ...
                Obx(
                  () => AppButton(
                    label: 'Verify OTP',
                    isLoading: authController.isVerifying.value,
                    loadingText: 'Verifying',
                    onPressed: _canSubmit
                        ? () {
                            if (_fieldOne.text.isNotEmpty) {
                              final request = OtpCheckRequest(
                                otp: _fieldOne.text.trim(),
                                phone: OTPScreen.phone,
                              );
                              authController.checkOtp(request);
                            }
                          }
                        : null,
                  ),
          ...
          onTap: () async {
            time == 0
                ? await showOtpHelpBottomSheet(
                    ...
                    onResend: () {
                      final request = ResendOtpRequest(phone: OTPScreen.phone);
                      authController.resendOtp(request);
                      // trigger resend code
                    },
                    onNeedHelp: () {
                          ...
                          children: [
                            ...
                            Obx(
                              () => Expanded(
                                child: AppButton(
                                  label: 'Resend again',
                                  isLoading: authController.isResending.value,
                                  onPressed: onResend,
                                ),
                              ),
                            ),
